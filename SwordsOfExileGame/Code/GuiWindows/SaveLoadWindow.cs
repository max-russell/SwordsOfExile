...
namespace SwordsOfExileGame
{
    ...
    delegate void FileHandler(int option, string filename);

    class LoadGameWindow : GuiWindow
    {
        public const int LOAD = 1, SAVE = 2, CANCEL = -1;

        PartyType curPartyTemp;

        ListBox savesList;
        RichTextBox saveInfoBox;
        Label saveName, titleLbl;
        PictureBox[] partyPics;
        Label[] partyNames = new Label[Constants.PC_LIMIT], 
                partyLvls = new Label[Constants.PC_LIMIT],
                partyStatus = new Label[Constants.PC_LIMIT];
        PictureBox scenPic;
        Button btnBack, btnLoad, btnSave, btnDelete;
        bool allowLoading, allowSaving, partiesOnly;
            ...
            : base(0, 0, 700, 500, true, false, true, true, false)
        {
            ...
            int x = 292, y = 52;
            for (int a = 0; a < Constants.PC_LIMIT; a++)
            {
        ...
        void confirmDelete(int n)
        {
            if (n == 0)
            {
                string f = Path.Combine(Game.RootDirectory, "Saves", Path.ChangeExtension(((SaveInfo)savesList.SelectedItem.Tag).Filename, "sav2"));
        ...
        void changeSave(bool user_caused, ListBoxItem item)
        {
            if (item.Tag == null)
            {
        ...
        void listSaves()
        {
            ...
            List<string> savegames = Directory.GetFiles(Path.Combine(Game.RootDirectory, "Saves")).ToList();
            ...
            foreach (string s in savegames)
            {
                ...
                SaveInfo i = Game.LoadSaveInfo(s2, partiesOnly);
                if (i != null) saves.Add(i);
            }

            foreach (SaveInfo i in saves)
            {
        ...
        void displaySaveInfo(SaveInfo i)
        {
            if (i == null)
            {
                saveName.Text = "";
