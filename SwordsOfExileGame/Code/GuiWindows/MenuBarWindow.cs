...
namespace SwordsOfExileGame
{
    ...
    class MenuBarWindow : GuiWindow
    {
        ...
        public static bool CombatEnabled { get { return instance.btnCombat.Enabled; } set { instance.btnCombat.Enabled = value; } }
        ...
        void pressAct(Control b) { new Action(eAction.Act); }
        void pressDefend(Control b) { new Action(eAction.Parry); }
        void pressRest(Control b) { new Action(eAction.StartRest); }
        void pressWait(Control b) { new Action(eAction.Wait); }
        void pressUse(Control b) { new Action(eAction.TargetUse); }
        void pressSave(Control b) { new Action(eAction.QuickSave); }
        void pressLoad(Control b) { new Action(eAction.QuickLoad); }

        void pressMagic(Control b)
        {
            if (b == btnCastMage)
        ...
        void pressAlchemy(Control b)
        {
            new Action(eAction.DoAlchemy);
        }

        void pressMap(Control b) { Gfx.MapZoom(); }
        void pressLook(Control b) { new Action(eAction.TargetSearch); }
        void pressStartCombat(Control button_pressed) { new Action(eAction.StartCombat); }
        void pressEndCombat(Control button_pressed) { new Action(eAction.EndCombat); }
        void pressTalk(Control button_pressed) { new Action(eAction.TargetTalk) { PC = Party.ActivePC }; }

        void pressGather(Control button_pressed)
        {
            new Action(eAction.GatherLoot);
        }

        void pressRanged(Control b)
        {
