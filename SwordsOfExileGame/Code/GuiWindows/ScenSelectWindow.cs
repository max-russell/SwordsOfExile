...
internal class ScenSelectWindow : GuiWindow
{
    private ListBox scenList;
    private RichTextBox scenInfoBox;
    private Label scenTitle;
    private PictureBox scenPic;
    private Button btnBack, btnNewParty, btnLoadParty;
        ...
        : base(0, 0, 700, 500, true, false, true, true, false)
    {
        ...
        if (scenList.Items.Count > 0)
        {
            ...
            foreach (var l in scenList.Items)
            {
    ...
    private void doLoadParty(int option, string filename)
    {
        if (option == LoadGameWindow.CANCEL)
            Visible = true;
        else
        {
            var i = Game.LoadSaveInfo(filename, true);
            if (i != null)
            {
    ...
    private void changeScenario(bool user_caused, ListBoxItem item)
    {
    ...
    private void listScenarios()
    {
        ...
        var scens_to_try = Directory.GetDirectories(Path.Combine(Game.RootDirectory, "Scenarios")).ToList();
        var scens = new List<ScenarioInfo>();

        //Try to load the info for each directory in the scenario folder. Only adding entry if directory is a valid scenario.
        foreach (var s in scens_to_try)
        {
            var si = ScenarioInfo.LoadScenarioInfo(s);
            if (si != null) scens.Add(si);
        }

        foreach (var si in scens)
        {
            scenList.AddItem(si.Name, Color.White, si, false);
        }
    }

    private void displayScenarioInfo(ScenarioInfo si)
    {
        if (si == null)
        {
