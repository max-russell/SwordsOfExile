...
{
    delegate void InputTextHandler(string text);

    delegate void InputKeyHandler(Keys k);
    class InputKeyWindow : GuiWindow
    {
        ...
        {
            Handler = handler;
            Label l = AddLabel(message, 10, 20, 260, -1, true);
            Position(-2,-2);
            Resize(300, 100);
            keyRemap = keyremap;
    ...
    class InputTextWindow : GuiWindow
    {
            ...
            : base(0, 0, width, 200, true, false, true, true, true)
        {
            returnText = handler;
            int ypos = 20;
            Label l = AddLabel(message, 10, ypos, width-40, -1, true);
            ypos += l.Height + 20;
        ...
        void pressCancel(Control button)
        {
            KillMe = true;
            returnText.Invoke(null);
        }

        class TextInputBox : Control
        {
            bool numbersOnly;
            public string Text;
            public int MaxChars = 30;
            int cursorpos = 0;
            bool cursorFlash, cursorTick;

            bool textHighlighting; //Whether user is dragging mouse to highlight a selection of text in the box
            int highlightPos; //Where the text highlight starts - cursorpos is the end.
            ...
            public override void Draw(SpriteBatch sb, int xOffset, int yOffset)
            {
                ...
                int lh = Maths.Min(cursorpos, highlightPos);
                int rh = Maths.Max(cursorpos, highlightPos);
            ...
            void mouseToCursorPos(int x, int y)
            {
                x = (Gui.Ms.X - x) - 4;
                y = (Gui.Ms.Y - y) - 2;
                ...
                {
                    int l = (int)Gfx.TalkFontNormal.MeasureString(Text.Substring(0, n)).Width;
