...
{
    class PortraitWindow : GuiWindow
    {
        PictureBox[] afflictBoxes = new PictureBox[14];

        public const int HEIGHT = 90;
        PictureButton inventoryButton, statsButton;
        public PCType PC;
        bool pressed, pressedLMB;
        ...
        public override void Draw(SpriteBatch sb, int partial = 0)
        {
            ...
            XnaRect r = new XnaRect(5, 145, 94, 92);
            ...
            if (PC.IsAlive())
            {
                ...
                if ((PC.GetSkill(eSkill.MAGE_SPELLS) > 0 || PC.GetSkill(eSkill.PRIEST_SPELLS) > 0))
                {
                    ...
                    int h = (int)(((float)PC.SP / (float)PC.MaxSP) * 64);
        ...
        public override bool Handle()
        {
            ...
            if (Gui.Ms.X >= dx && Gui.Ms.Y >= dy && Gui.Ms.X < dx + 64 && Gui.Ms.Y < dy + 64) //Mouse within bounds of face box?
            {
                ...
                else if (!Gui.LMBDown && !Gui.RMBDown)
                {
                    ...
                    {
                        foreach (PictureBox pb in afflictBoxes)
                        {
