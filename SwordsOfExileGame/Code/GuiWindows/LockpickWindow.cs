...
internal class LockpickWindow : GuiWindow
{
    private Location doorPos;
    private List<OptionPictureButton> pc_buttons = new();
    private List<PCType> pcs = new();
    private OptionButton bashButton, pickButton;
    private Button okButton, cancelButton;
    private Label chanceLabel, additionalLabel;
    private List<PictureBox> hasLockpicks = new();
        ...
        : base(0, 0, 270, 270, true, false, true, true, true)
    {
        ...
        var dr = new XnaRect(20, 50, Gfx.PCPORTRAITWIDTH, Gfx.PCPORTRAITHEIGHT);
    ...
    private void pressCancel(Control button_pressed) { KillMe = true; }

    private void pressDoIt(Control button_pressed)
    {
        var pc = pcs[0];
        for (var n = 0; n < pc_buttons.Count; n++)
            if (pc_buttons[n].Pressed) { pc = pcs[n]; break; }
    ...
    private void pressTactic(Control button_pressed)
    {
        setChanceInfo();
    }
