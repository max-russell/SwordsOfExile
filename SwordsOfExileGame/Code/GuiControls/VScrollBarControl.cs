...
internal class VScrollBar : Control
{
    private Control Owner;

    private int pos, extend, min, max, Change;
    private int YOffset;
    ...
    public override void Draw(SpriteBatch sb, int xOffset, int yOffset)
    {
        ...
        if (Enabled)
        {
            ...
            var g1 = ((float)pos / (float)max) * barh;
            var g2 = ((float)extend / (float)max) * barh;
        ...
        if (Gui.Ms.X >= dx && Gui.Ms.X < dx + 16 && Gui.Ms.Y >= dy && Gui.Ms.Y < dy + Height)
        {
            ...
            else //Bar
            {
                if (Gui.LMBDown)
                {
                    ...
                    var mpos = (float)(Gui.Ms.Y - dy - 16) / (float)(Height - 32);
