...
namespace SwordsOfExileGame
{
    public partial class PartyType : IExpRecipient
    {
        ...
        int gold = Constants.STARTING_GOLD, food = Constants.STARTING_FOOD;
          public int Gold { get { return gold; } set { gold = Maths.MinMax(0, Constants.GOLD_LIMIT, value); } }
          public int Food { get { return food; } set 
          { food = Maths.MinMax(0, Constants.FOOD_LIMIT, value); } }
        ...
        public void SetUpKnownRecipes()
        {
            ...
            {
                string s = KnownRecipes.ElementAt(n).Key;
        ...
        public long total_dam_taken = 0;
          ...
          IMap currentMap { get { return Game.CurrentMap; } set { Game.CurrentMap = value; } }
        ...
        public Location Pos {
            ...
            set
            {
                ...
                //    case eMode.COMBAT:
                //    case eMode.TOWN:
                        foreach (PCType pc in PCList) pc.Pos = value;
                        //break;
                //    case eMode.OUTSIDE:
                //        OutsidePos = value;
                //        break;
        ...
        public Direction Direction
        {
            ...
            set
            {
                ...
                //    case eMode.COMBAT:
                //    case eMode.TOWN:
                        foreach (PCType pc in PCList) pc.Dir = value;
                //        break;
                //    case eMode.OUTSIDE:
                //        outsideDir = value;
                //        break;
        ...
        public void Restore()
        {
            ...
            foreach (PCType pc in PCList)
            {
                if (pc.LifeStatus != eLifeStatus.ABSENT)
                {
                    ...
                    pc.SP = pc.MaxSP;
                    for (int a = 0; a < pc.status.Length; a++)
                        pc.status[a] = 0;
                }
            }
        }
        public void StripItems()
        {
            ...
            foreach (PCType pc in PCList)
            {
                for (int a = pc.ItemList.Count - 1; a >= 0; a--)
                {
                    Item item = pc.ItemList[a];
                    item.SpecialClass = 0;
        ...
        public IEnumerable<PCType> EachAlivePC() {
            foreach (PCType pc in PCList) if (pc.IsAlive()) yield return pc;
        }
        ...
        {
            List<PCType> live_pcs = PCList.FindAll(n => n.LifeStatus == eLifeStatus.ALIVE);
        ...
        public int NoOfLivingPCs
        {
            ...
            {
                int amt = 0;
                foreach (PCType pc in EachAlivePC()) amt++;
                return amt;
            }
        }
        ...
        public PCType ClosestPC(Location where) {
            ...
            foreach (PCType pc in EachIndependentPC()) //(i = 0; i < 6; i++)
        ...
        public int CountItemClass(int cls, bool and_remove)
        {
            ...
            foreach (PCType pc in EachAlivePC())
            {
                for (int n = pc.ItemList.Count - 1; n >= 0; n--)
        ...
        public void DamageActive(int how_much, eDamageType dam_type)
        {
            ...
            else foreach (PCType pc in EachAlivePC())
        ...
        public bool IncreaseAge() //Return true if a timer has triggered a script.
        {
	        ...
	        // Protection, etc.
	        foreach(PCType pc in EachAlivePC()) { // Process some status things, and check if stats updated
	        ...
	        if (Age % 4 == 0)
                foreach(PCType pc in EachAlivePC())
                {
                    ...
                    {
					        int j = Maths.Rand(1,0,item.AbilityStrength / 3);
        ...
        void HandleAcid()
        {
            ...
            foreach (PCType pc in EachAlivePC())
            {
                int p = pc.Status(eAffliction.ACID);
                if (p > 0)
                {
                ...
                if (i == 50)
                {
                    if (Constants.COMPATIBILITY_CHECK_TIMERS_WHILE_RESTING)
                    {
                        for (i = 0; i < 115; i++)
                        {
                            ...
                            {
                                i = 200;
                                foreach (PCType pc in EachAlivePC()) pc.Disease(2);
                            }
                            // Plants and magic shops
                            Shop.RestockAll(Age);
