...
public partial class Scenario
{
    ...
    public static bool Load() {
        ...
        var filename = Path.ChangeExtension(Filename, "exs2");
    ...
    static public int difficulty_adjust()
    {
        ...
        foreach (var pc in Party.EachAlivePC())// (i = 0; i < 6; i++)
            j += pc.Level;
...
internal class ScenarioInfo
{
    ...
    public static ScenarioInfo LoadScenarioInfo(string fn)
    {
        ...
        var filename = Path.Combine(Game.RootDirectory, "Scenarios", fn, Path.ChangeExtension(fn, "exs2"));
        if (!File.Exists(filename)) return null;

        var si = new ScenarioInfo();
