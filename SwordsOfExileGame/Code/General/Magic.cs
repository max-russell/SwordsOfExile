...
public class MagicSpell : IListEntity
{
    ...
    public static ExileList<MagicSpell> List = new();
    ...
    public void Cast(PCType caster, object targ, Item item_used)
    {
        ...
        var castbyitemuse = item_used != null;
        ...
        var target = Location.Zero;
        var targetpattern = Action.TargetPattern; //Don't use the current spell's TargetPattern property - Wall of Force etc lets the player change the pattern when targeting!
        ...
        //Check map triggers
        if (targ is Location or List<Location>)
        {
            List<Location> locs;
            if (targ is Location)
            {
            ...
            foreach (var l in locs)
            {
                ...
                foreach (var se in Game.CurrentTown.TriggerSpotList)
                {
                ...
                {
                    var ter = Game.CurrentTown.TerrainAt(l);
