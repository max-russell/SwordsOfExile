...
internal class Script
{
    ...
    //Script thread flags
    private static bool scriptsbegun = false;
    private static bool scriptsstopped = false;
    private static bool scriptsfinished = false;
    private static SemaphoreSlim _sem = new(1);
    private static bool latentScriptRunning = false; //Flag to prevent Wait() being called from script that isn't latent.

    //Python stuff
    private static ScriptEngine scrEngine;
    private static ScriptScope scrScope;
    private static ScriptScope builtinScope;

    #region Latent Script Function Creators

    private Script(eCallOrigin origin, string func) 
    {
    ...
    {
        if (func is null or "") return;
        var s = new Script(origin, func);
    }
    public static void New_Talking(string func, NPC npc)
    {
        if (func is null or "") return;
        var s = new Script(eCallOrigin.TALKING, func);
        s.Params.NPCTarget = npc;
    }

    public static void New_ExitTown(string func, Direction dir)
    {
        var s = new Script(eCallOrigin.LEAVING_TOWN, func);
        s.Params.Dir = dir;
    }
    ...
    {
        if (tr.Func is null or "") return;
        if (ch is PCType)
        {
            var s = new Script(eCallOrigin.PC_STOOD_ON, tr.Func);
            s.Params.PC = (PCType)ch;
            s.Params.Vars = tr.Vars;
        }
        else if (ch is NPC)
        {
            var s = new Script(eCallOrigin.NPC_STOOD_ON, tr.Func);
    ...
    {
        if (func is null or "") return;
        var s = new Script(eCallOrigin.KILLED_NPC, func);
        s.Params.NPCTarget = npc;
        s.Params.PC = pc;
    }
    ...
    {
        if (func is null or "") return;
        var s = new Script(eCallOrigin.USING_SPECIAL_ITEM, func);
        s.Params.SItem = si;
    }
    ...
    {
        if (func is null or "") return;
        var s = new Script(item == null ? eCallOrigin.CAST_SPELL : eCallOrigin.CAST_ITEM_SPELL, func);
    ...
    {
        if (trigger.Func is null or "") return;
        var s = new Script(item == null ? eCallOrigin.CAST_SPELL : eCallOrigin.CAST_ITEM_SPELL, trigger.Func);
        s.Params.PC = caster;
        s.Params.Target = loc;
        s.Params.Spell = spell;
        s.Params.UsedItem = item;
    ...
    private void scriptThreadRoutine()
    {
    ...
    public bool Run()
    {
        if (!scriptsstopped)
        {
            ...
            var h = new Thread(scriptThreadRoutine);
            h.Start();
    ...
    private void Finish()
    {
        ...
        else if (Params.Origin is eCallOrigin.CAST_SPELL or eCallOrigin.CAST_ITEM_SPELL)
        {
    ...
    public static bool Initialise()
    {
        ...
        var source = scrEngine.CreateScriptSourceFromString("from SwordsOfExileGame import *", SourceCodeKind.SingleStatement);
        var comp = source.Compile();
        ...
        for (var n = 0; n < scenscriptfiles.Count; n++)
        ...
        foreach (var s in basescriptfiles)
        {
        ...
        foreach (var s in scenscriptfiles)
        {
    ...
    /// </summary>
    public static bool IsRunning => (ScriptQueue.Count > 0);
    ...
    {
        var f = scrScope.GetVariable<Func<NPC, bool>>(func);
        builtinScope.SetVariable("Party", Game.CurrentParty);
        builtinScope.SetVariable("Town", Game.CurrentTown);
        builtinScope.SetVariable("WorldMap", Game.WorldMap);
        try { return f(npc); }
    ...
    {
        if (func is null or "") return town;
        var f = scrScope.GetVariable<Func<TownMap, TownMap>>(func);
        builtinScope.SetVariable("Party", Game.CurrentParty);
        builtinScope.SetVariable("WorldMap", Game.WorldMap);
        try { return f(town); }
    ...
    {
        if (func is null or "") return false;
        var f = scrScope.GetVariable<Func<Shop, bool>>(func);
        builtinScope.SetVariable("Party", Game.CurrentParty);
        builtinScope.SetVariable("Town", Game.CurrentTown);
        builtinScope.SetVariable("WorldMap", Game.WorldMap);
        try { return f(shop); }
    ...
    private static void FlagScriptError(Exception e)
    {
        var eo = scrEngine.GetService<ExceptionOperations>();
    ...
    private static void script_Wait()
    {
    ...
    private static void script_SmallMessage(string s)
    {
        Game.AddMessage(s);
    }

    private static void script_Message(string s)
    {
        if (!latentScriptRunning) return; //Nothing happens if script is non-latent.
    ...
    private static int script_Choice(string s, eDialogPic pictype, int picnum, PyList buttons)
    {
        if (!latentScriptRunning) return -1; //Nothing happens if script is non-latent.
        Result = -1;

        var b_array = new string[buttons.Count];
    ...
    private static PCType script_SelectPC(string s, bool only_living = true)
    {
    ...
    private static string script_InputText(string msg, string def)
    {
        if (!latentScriptRunning) return ""; //Nothing happens if script is non-latent.
        TextResult = "";
    ...
    private static int script_InputNumber(string msg, int def)
    {
        if (!latentScriptRunning) return -1; //Nothing happens if script is non-latent.
        Result = -1;
    ...
    private static bool script_RunScript(string func, ScriptParameters s)//, Location pos)
    {
    ...
    private static void script_OpenShop(string id)
    {
        if (!latentScriptRunning) return; //Nothing happens if script is non-latent.
    ...
    private static void script_OpenHealingShop(int pricelevel)
    {
        if (!latentScriptRunning) return; //Nothing happens if script is non-latent.
    ...
    private static void script_OpenTrainingShop()
    {
        if (!latentScriptRunning) return; //Nothing happens if script is non-latent.
    ...
    private static void script_OpenIdentifyShop(string shopname, int price)
    {
        if (!latentScriptRunning) return; //Nothing happens if script is non-latent.
    ...
    private static void script_OpenEnchantShop(string shopname, eEnchantShop enchant_type)
    {
        if (!latentScriptRunning) return; //Nothing happens if script is non-latent.
    ...
    private static void script_CentreView(Location l, bool instant)
    {
        Gfx.CentreView(l, instant);
    }

    private static void script_SuspendMapUpdate()
    {
        suspendMapUpdate = true;
    }

    private static void script_ResumeMapUpdate()
    {
    ...
    private static void defaultTextHandler(string s)
    {
        TextResult = s;
    }

    private static void defaultNumberHandler(string s)
    {
        Result = Convert.ToInt32(s);
    }

    private static void defaultHandler(int button_index)
    {
        Result = button_index;
    }

    private static void defaultPCHandler(PCType pc)
    {
        SelectedPC = pc;
    }
    #endregion
    ...
    private static PCType SelectedPC = null;
    ...
    private static void ResetInterfaceValues()
    {
        Result = -1;
        SelectedPC = null;
    }
