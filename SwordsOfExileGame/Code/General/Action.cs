...
namespace SwordsOfExileGame
{
    public class Action
    {
        ...
        //Shortcuts
        PartyType CurrentParty = Game.CurrentParty;
        IMap CurrentMap = Game.CurrentMap;
        TownMap CurrentTown = Game.CurrentTown;
        WorldMapType WorldMap = Game.WorldMap;
        eMode Mode = Game.Mode;
        void AddMessage(string m) { Game.AddMessage(m); }
         ...
         public eState _Handle()
        {
            ...
            switch (action)
            {
                ...
                case eAction.GatherLoot:
                    LootSpot loot = LootSpot.Generate(CurrentParty.Pos, (TownMap)CurrentMap, true);
                ...
                case eAction.TakeAllItemMap:
                    ...
                    foreach (Item item in CurrentTown.EachItemThere(Loc))
                        removelist.Add(item);
        ...
        {
            bool finishspell = false; //For when player presses 'space' to finish a multi-target spell without using all targets.
                ...
                case eAction.Space:
                    ...
                    if (TargetSpell != null)
                    {
                        if (TargetSpell.MultiTarget)
                            ...
                            for (int n = 0; n < Pattern.Field.Length; n++)
            ...
            {
                int num = Requested - eAction.TargetLetterSelectA;
                NPC npc = CurrentTown.FindTargetLetterNPC(num);
                target = npc.Pos;
            }
            ...
            if (!finishspell)
            {
                if (Game.Mode != eMode.OUTSIDE)
                {
                    ...
                    if ((TargetWhat == eAction.ItemSpellTargeting || TargetWhat == eAction.SpellTargeting)
                ...
                if (TargetNPCsOnly)
                {
                    NPC npc = CurrentTown.CharacterThere(target) as NPC;
