...
{
    public static ExileList<Personality> List = new();
    ...
    public void Load(BinaryReader In)
    {
        ...
        int n = In.ReadInt16();
        for (var x = 0; x < n; x++) TalkingNodeList.Add(new TalkingNode(In));
    ...
    public TalkingNode FindTalkingNode(string word)
    {
        ...
        foreach (var node in TalkingNodeList)
        {
...
public class TalkingNode
{
    ...
    public TalkingNode(BinaryReader In)
    {
        ...
        if (Type is eTalkNodeType.RUN_HEALER or eTalkNodeType.RUN_ENCHANTING)
