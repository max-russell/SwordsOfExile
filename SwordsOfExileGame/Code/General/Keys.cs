...
{
    //Keyboard Routines
    private static KeyboardState testKeys, oldKeys;
    ...
    public static bool AnyKeysHit(params Keys[] ka) { foreach (var k in ka) if (KeyHit(k)) return true; return false; }
    public static bool KeyDown(Keys k) { return testKeys.IsKeyDown(k); }
    public static bool AnyKeysDown(params Keys[] ka) { foreach (var k in ka) if (testKeys.IsKeyDown(k)) return true; return false; }
    public static void GetKeysHit(int elapsedtime)
    {
        ...
        var nk = testKeys.GetPressedKeys();
        var ok = oldKeys.GetPressedKeys();
    ...
    public static void GetActionKeys()
    {
        ...
        if (!Action.Exists() && hitKey != Keys.None)
        {
            ...
            else if (hitKey is Keys.Oem8 or Keys.OemTilde && Action.LockActions == eAction.NONE && !Gui.WorldModalPaused)
            {
