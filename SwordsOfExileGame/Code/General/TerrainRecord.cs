...
public class TerrainRecord : IListEntity 
{
    ...
    public static TerrainRecord NoOverlay = new();
    ...
    public static ExileList<TerrainRecord> List = new();

    private const int CAVE_WALKWAY = 82;
    private const int GRASS_WALKWAY = 83;

    private const int CAVE_ROAD = 79,
        GRASS_ROAD = 80,
        HILLS_ROAD = 81,
        GRASS_S_HILLS_N = 38,
        GRASS_E_HILLS_W = 40,
    ...
    public static void LoadAll(BinaryReader In)
    {
        ...
        var trans_to_ids = new List<string>();
        ...
        var x = 0;
        foreach (var ter in TerrainRecord.List)
        {
            if (trans_to_ids[x] != "" && TerrainRecord.List.Contains(trans_to_ids[x]))
                ter.TransformTo = TerrainRecord.List[trans_to_ids[x]];
            x++;

            if (ter.Special is eTerSpec.CHANGE_WHEN_STEP_ON or eTerSpec.CRUMBLING_TERRAIN or eTerSpec.LOCKABLE_TERRAIN or eTerSpec.UNLOCKABLE_TERRAIN or eTerSpec.UNLOCKABLE_BASHABLE or eTerSpec.CHANGE_WHEN_USED)
            {
                var s = (string)ter.Flag1;
    ...
    public void Draw(SpriteBatch sb, XnaRect r_dst, bool fullbright)//, TerrainRecord to_n = null, TerrainRecord to_s = null, TerrainRecord to_w = null, TerrainRecord to_e = null)
    {
        ...
        else
        {
            ...
            var col = no % Gfx.TerrainGfxSlotsAcross[sheet];
            var row = no / Gfx.TerrainGfxSlotsAcross[sheet];
    ...
    {
        if (Special is eTerSpec.UNLOCKABLE_TERRAIN or eTerSpec.UNLOCKABLE_BASHABLE
    ...
    public bool DoNotPlacePC => Blockag >= eBlock2.CLEAR_NO_PLACE;
    public bool DoNotPlaceNPC => Blockag >= eBlock2.CLEAR_PC;

    public bool BlocksPC => Blockag == eBlock2.BLOCKED; ////Blockage > eBlock.CLEAR_WALK_PC; } }
    public bool BlocksNPC => Blockag is eBlock2.BLOCKED or eBlock2.CLEAR_PC; //Blockage >= eBlock.CLEAR_WALK_PC; } }
    ...
    public void NastyTerrainEffect(PCType pc, bool report = true)
    {
        switch (Special)
        {
            ...
            case eTerSpec.DOES_MAGIC_DAMAGE:
                ...
                var dam_type = eDamageType.WEAPON; var pic_type = 0;
                ...
                var r1 = Maths.Rand(Convert.ToInt32(Flag2), 1/*(int)dam_type*/, Convert.ToInt32(Flag1));
